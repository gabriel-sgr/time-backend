...
router.post('/text', protect, async (req, res) => {
  try {
    const { title, content, expires_at, fontSize } = req.body;
    if (!content) return res.status(400).json({ message: 'Content is required for text announcements' });
    const announcement = await Announcement.create({
      type: 'text',
      title: title || '',
...
// PUT update text announcement
router.put('/:id', protect, async (req, res) => {
  try {
    const ann = await Announcement.findById(req.params.id);
    if (!ann) return res.status(404).json({ message: 'Announcement not found' });
    
    // Only allow updating text announcements
    if (ann.type !== 'text') return res.status(400).json({ message: 'Only text announcements can be updated' });
    
    const { title, content, expires_at, fontSize } = req.body;
    
    if (title !== undefined) ann.title = title;
    if (content !== undefined) ann.content = content;
    if (expires_at !== undefined) ann.expires_at = expires_at;
    if (fontSize !== undefined) ann.fontSize = fontSize;
